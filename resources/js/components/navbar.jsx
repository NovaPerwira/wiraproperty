...
const navItems = [
  {
    label: "About",
    bgColor: "rgba(255,255,255,0.03)", // Transparan untuk efek glass
    textColor: "#fff",
    links: [
  ...
  },
  {
    ...
    textColor: "#fff",
    links: [
      { label: "Email Us", href: "/email", ariaLabel: "Email us" },
                ...
                {item.links?.map((lnk, i) => {
                  ...
                  return (
                    <a
                      key={`${lnk.label}-${i}`}
                      href={lnk.href}
                      onClick={(e) => { e.preventDefault(); onNavigate(lnk.href); toggleMenu(); }}
              ...
              <div className="flex items-center space-x-2 h-full">
                {items.map((category) => {
                  ...
                  return (
                    ...
                    >
                      <a href={category.links[0]?.href} onClick={(e) => { e.preventDefault(); onNavigate(category.links[0]?.href); }} className="relative px-5 py-2.5 rounded-full flex items-center gap-1.5 transition-colors">
                        {isActive && (
                          ...
                          >
                            <div className="flex flex-col gap-1.5 p-3">
                              ...
                              {category.links.map((lnk) => {
                                 ...
                                 return (
                                   <a key={lnk.href} href={lnk.href} onClick={(e) => { e.preventDefault(); onNavigate(lnk.href); setHoveredNav(null); }} className={`flex items-center justify-between p-3.5 rounded-xl transition-all duration-300 group/link ${isLinkActive ? 'bg-white/20 text-white border border-white/10 shadow-sm' : 'hover:bg-white/[0.08] border border-transparent text-white/70 hover:text-white'}`}>

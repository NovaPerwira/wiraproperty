...
const STATUS_COLORS: Record<string, { bar: string; text: string }> = {
    ...
    cancelled: {
        bar: 'bg-red-400 opacity-50 line-through',
        text: 'text-white',
    },
};
            ...
            <div className="flex flex-col gap-4 p-6">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Visual room booking layout — {bookings.length}{' '}
                            bookings this month
                        </p>
                    </div>
                    {/* Month navigation */}
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => navigate(-1)}
                            className="flex h-9 w-9 items-center justify-center rounded-lg border border-input text-lg hover:bg-muted"
                        >
                            ‹
                        </button>
                ...
                <div className="flex flex-wrap gap-3">
                    {Object.entries(STATUS_COLORS).map(([k, v]) => (
                        <span
                            key={k}
                            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1 text-xs font-semibold text-white ${v.bar.split(' ')[0]}`}
                        >
                            {k.replace('_', ' ')}
                        </span>
                    ))}
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-red-200 px-3 py-1 text-xs font-semibold text-red-900">
                        🔧 Maintenance
                    </span>
                </div>
                ...
                <div className="overflow-hidden rounded-xl border border-sidebar-border/70 bg-white shadow-sm dark:border-sidebar-border dark:bg-sidebar">
                    ...
                    >
                        <div
                            style={{
                                minWidth: `${140 + CELL_W * daysInMonth}px`,
                            }}
                        >
                            {/* Day headers */}
                                ...
                                {Array.from({ length: daysInMonth }, (_, i) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative flex flex-col items-center justify-center border-r border-sidebar-border/30 py-1.5 dark:border-sidebar-border ${isToday ? 'bg-primary/10' : isWeekend ? 'bg-muted/40' : ''}`}
                                        >
                                            <span
                                            ...
                                            {isToday && (
                                                <div className="absolute bottom-0 left-1/2 h-0.5 w-3 -translate-x-1/2 rounded bg-primary" />
                                            )}
                                        </div>
                                    );
                                })}
                                    ...
                                    <div key={floor}>
                                        ...
                                        {floorRooms.map((room) => {
                                            ...
                                            return (
                                                <div
                                                    key={room.id}
                                                    className="group relative flex border-b border-sidebar-border/30 hover:bg-muted/20 dark:border-sidebar-border"
                                                    style={{ height: ROW_H }}
                                                >
                                                    {/* Room label */}
                                                    ...
                                                    >
                                                            ...
                                                            (_, i) => {
                                                                ...
                                                                return (
                                                                    <div
                                                                        ...
                                                                        className={`h-full border-r border-sidebar-border/20 dark:border-sidebar-border ${isToday ? 'bg-primary/5' : isWeekend ? 'bg-muted/30' : ''}`}
                                                                    />
                                                                );
                                                            },
                                                        )}
                                                        ...
                                                        {isMaintenance && (
                                                            ...
                                                            >
                                                                <div className="flex h-6 w-full items-center border-y border-red-300 bg-red-200/80 px-2 dark:border-red-700 dark:bg-red-900/40">
                                                                ...
                                                                (b) => {
                                                                    ...
                                                                    return (
                                                                        <div
                                                                            ...
                                                                            onClick={() =>
                                                                                router.visit(
                                                                                    `/admin/bookings?search=${encodeURIComponent(b.guest_name)}`,
                                                                                )
                                                                            }
                                                                        >
                ...
                {tooltip && (
                    ...
                    >
                        <div className="mb-2 flex items-center gap-2">
                            <span
                                className={`inline-flex rounded-full px-2 py-0.5 text-xs font-bold text-white ${STATUS_COLORS[tooltip.booking.status]?.bar.split(' ')[0]}`}
                            >
                                {tooltip.booking.status.replace('_', ' ')}
                            </span>
                            <span className="text-xs text-muted-foreground">
                                {SOURCE_LABELS[tooltip.booking.booking_source]}
                            </span>
                        </div>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                            ...
                            <div>
                                ...
                                <div className="font-medium">
                                    {tooltip.booking.check_out_date}
                                </div>
                            </div>
                            <div>
                                <span className="text-xs text-muted-foreground">
                            ...
                            <div>
                                ...
                                <div className="font-semibold text-emerald-600">
                                    {fmt(tooltip.booking.total_amount)}
                                </div>
                            </div>
                        </div>
